	...
	return {
	  ...
	  link: function (scope, element, attrs) {
		...
		var LiteMolComponent = (function () {
			...
			LiteMolComponent.prototype.colorResidues = function(residues, colorCode, showSideChains, showLigands) {
				_this = this;
				var visual = this.selectNodes('polymer-visual')[0];
				if (!visual)
					return;
				this.resetThemeSelHighlight(); //clear selection
				var query = this.Query.residuesById.apply(void 0, residues);
				var theme = this.createSelectionTheme(this.CoreVis.Color.fromRgb(colorCode.r, colorCode.g, colorCode.b));
				var action = this.Transform.build()
					.add(visual, this.Transformer.Molecule.CreateSelectionFromQuery, { query: query, name: 'My name' }, { ref: 'sequence-selection' })
				...
				// show water
				var waterVisual = this.selectNodes('water-visual')[0];
				if (waterVisual) {
					_this.Command.Entity.SetVisibility.dispatch(_this.plugin.context, { entity: waterVisual, visible: false })
				}

				// show ligands
				var ligandVisual = this.selectNodes('het-visual')[0];
				if (ligandVisual) {
					_this.Command.Entity.SetVisibility.dispatch(_this.plugin.context, {
						entity: ligandVisual,
						visible: showLigands
					})
				}
